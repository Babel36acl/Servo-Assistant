               ...
               boolean CA, int *psize, void *p, int timeout)
{
   ...
   boolean NotLast;
   int capacity = *psize;
   osal_timert transfer_timer;
   osal_timer_start(&transfer_timer, 2000000);
   ...
   if (wkc > 0) /* succeeded to place mailbox in slave ? */
   {
      ...
      if (wkc > 0) /* succeeded to read slave response ? */
      {
         ...
         if (etohs(aSDOp->MbxHeader.length) >= 10 && etohs(aSDOp->MbxHeader.length) <= EC_MAXMBX - 6 &&
             ((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) &&
             ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) &&
              (aSDOp->Index == SDOp->Index) && (aSDOp->SubIndex == subindex) &&
              ((aSDOp->Command & 0xe0) == 0x40))
         {
               ...
               if (SDOlen >= 0 && SDOlen <= *psize)
               {
                  ...
                  if (Framedatasize < SDOlen) /* transfer in segments? */
                  {
                     ...
                     *psize = Framedatasize;
                     ...
                     {
                        if (osal_timer_is_expired(&transfer_timer)) { wkc = 0; break; }
                        SDOp = (ec_SDOt *)&MbxOut;
                        SDOp->MbxHeader.length = htoes(0x000a);
                        SDOp->MbxHeader.address = htoes(0x0000);
                        SDOp->MbxHeader.priority = 0x00;
                           ...
                           if (wkc > 0)
                           {
                              ...
                              if ((((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) &&
                                   ...
                                   ((aSDOp->Command & 0xe0) == 0x00)))
                                        {
                                 /* calculate mailbox transfer size */
                                 if (etohs(aSDOp->MbxHeader.length) < 3 || etohs(aSDOp->MbxHeader.length) > EC_MAXMBX - 6 || (aSDOp->Command & 0x10) != toggle) { wkc = 0; break; }
                                 Framedatasize = etohs(aSDOp->MbxHeader.length) - 3;
                                 if (Framedatasize == 7 && (aSDOp->Command & 1)) Framedatasize -= ((aSDOp->Command & 0x0e) >> 1);
                                 if (Framedatasize > capacity - *psize || Framedatasize > SDOlen - *psize || (!Framedatasize && !(aSDOp->Command & 1))) { wkc = 0; break; }
